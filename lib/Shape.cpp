...
namespace rt {
    ...
    bool Cylinder::check_cap(const Ray& ray, const double t) {
    ...
    std::vector<Intersection> Cone::local_intersect(const Ray& ray) {
        std::vector<Intersection> xs;
    ...
    void Cone::intersect_caps(const Ray& ray, std::vector<Intersection>& xs) {
        ...
        auto t = (minimum_ - ray.origin().y) / ray.direction().y;
        if (check_cap(ray, t, minimum_)) {
            xs.emplace_back(t, this);
        }

        t = (maximum_ - ray.origin().y) / ray.direction().y;
        if (check_cap(ray, t, maximum_)) {
            xs.emplace_back(t, this);
        }
    }

    bool Cone::check_cap(const Ray& ray, const double t, const double y) {
        const auto x = ray.origin().x + t * ray.direction().x;
        const auto z = ray.origin().z + t * ray.direction().z;
    ...
    std::unique_ptr<Shape> cone(double minimum, double maximum, bool closed) {
        return make_unique<Cone>(minimum, maximum, closed);
    }
}
